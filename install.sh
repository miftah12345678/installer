...
display_message() {
    ...
    echo ""
    echo -e "${RED}"
    echo "HAMS OFFC SOLO ALL"
    echo -e "${RESET}"
    echo ""
    echo ""
    echo -e "${BLUE}                    WHATSAPP : +62 857-8154-9773${RESET}"
    echo -e "${YELLOW}                    YOUTUBE  : @hamstore23${RESET}"
    echo ""
}
...
if check_license; then
    ...
    if [ $? -eq 0 ]; then
        display_message
        install_software
        animate_text "POWERED BY HAMSOFFC"
    else
